...
int List::deleteNode(int const data)
{
	if (isEmpty())
	{
...
{
	return *(squaredBrackets(index));
}

int List::getNumber(int const index) const
{
	return squaredBrackets(index)->data;
}

Node * List::squaredBrackets(int const index) const
{
	int i = 0;
	auto* temp = head;
